...
//	public Conta_Pagar d (Conta_Pagar cp) {
//		cp.setNome(cp.getNome());
////		cp.setData_entrada(LocalDate.of(cp.getData_entrada()));
////		cp.setData_vencimento(cp.getData_vencimento());
//		cp.setFornecedor(cp.getFornecedor());
//		cp.setNumero_nota(cp.getNumero_nota());
//		cp.setValor(cp.getValor());
//		
//		return conta_pagarRepository.save(cp);
//	}
	...
	public ModelAndView salvarConta_Pagar(@Valid Conta_Pagar conta_pagar, BindingResult result) {
		...
		try {
			//conta_pagar.setData_entrada().
			conta_pagarRepository.saveAndFlush(conta_pagar);
